...
4. 演练前必须解决安全语义冲突：参考包 `pcs.setpoint` 标记 `highRisk: true`，而 ADR-0007
   要求高风险命令逐次人工二次确认；当前已启用的 EMS 策略因此稳定返回
   `CONTROL_CONFIRMATION_REQUIRED`，无法得到自动控制回读。不能通过测试夹具绕过；需要维护者
   确认“高风险自动化应继续禁止”还是新增经工程师显式启用、配置摘要绑定且可撤销的策略授权契约。
