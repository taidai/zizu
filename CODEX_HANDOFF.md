...
## Session 2026-08-29 — L0 实时页面自恢复修复（本地候选，未部署）
...
- TDD 证据：新增两个恢复测试及一个状态区分断言，先按预期失败，再转绿。前端全量 44/44 通过；
  TypeScript/Vite production build 通过，8185 modules，只有既有大 chunk 警告。
- 当前改动尚未提交、推送、构建镜像或部署 1 号机；线上仍为 v0.4.87。数据库连接池耗尽的服务端根因仍需
  独立诊断，本修复只保证短暂故障不会让页面永久停在假“设备超时”状态。
