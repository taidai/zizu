...
## Session 2026-09-08 — v1.0.6 通用策略专项验收

- 新鲜执行本机真实纵向验收3/3：浏览器新建/绑定/原生编辑/试算/发布/启停，经真实PostgreSQL、GoRules、committed L2与统一控制验证；协议边界为隔离模拟器，不是现场PCS。覆盖回读确认、失败锁定停用、同分钟不重发与主干同一实体身份。随机测试库已清理，匹配残留数0；首次因未传默认DB_USER在初始化前失败，补齐调用环境后通过。
- 原生JDM契约15/15通过，含多个有序控制意图与不合法输出拒绝。
- 现场检查6条策略：4条无输入/输出，包含当前“通用调度策略”空草稿；另2条旧策略受L2_CONFIGURATION_MISMATCH保护阻断。不能因此宣称全部现场策略已可运行。
- 仅通过浏览器重新保存一条已存在、停用的验收专用策略草稿，将基线777刷新到780；验证JDM内容与输入/输出身份不变、已发布版本不变、仍停用。随后试算HTTP200/EVALUATED，有命中规则及拟议目标；页面/API错误0，无控制下发。另一条旧策略及空草稿未改。
- 本轮未改产品代码、未发布新镜像。私有证据位于 `.release-artifacts/v1.0.6/generic-real-tests-2.log`、`generic-native-tests.log`、`generic-field/`、`generic-field-refresh/`。使用具体策略仍需正确绑定、定义条件/目标、试算并发布；真实启用须遵守现场授权。

## Session 2026-09-08 — v1.0.6 实体新鲜度与调度 Demo 布局（已部署、现场复验通过）

- 现场只读定位：同一 Neuron 来源允许导入到多个节点，但采集字典只保留最后一个点位，导致其他已绑定实体长期 INPUT_STALE；真实 PCS/BMS 当前仍有新数据，不是整个 MQTT/数据库链路停止。
- 修复采集映射为每个已启用节点点位独立分发，保留原始值、独立身份/类型/质量和来源排序；不改 STALE 阈值、不删除或搬迁实体、不修改控制边界。
- 真实 parser→adapter→blackboard 回归先失败后通过，覆盖重复导入两种顺序、同值新时间、迟到丢弃；backend 全量 774 项=493通过+281条件跳过。独立只读复审无关键/重要问题，pipeline 6/6。
- 调度页面已按已确认 Demo 重排为横向三步摘要、全宽原生决策表、输入/输出暂存弹窗；预览/JSON/表达式说明按需弹出。沿用现有保存/试算/发布/控制安全逻辑，关键事件区保持原功能。取消不改变草稿，原生同步屏障未改。
- 前端201/201、scripts56/56、production build8211模块通过。受影响浏览器26项首轮25通过，唯一失败为新增测试误将认证ws-ticket计入业务写；限定策略/控制写入后该项单独通过，不冒充一次26/26。1024/1280截图已实看；前后端独立复审无关键/重要问题。
- 源码/tag `v1.0.6` 为 `340f57911516c89518485aae33977be06ad27d41`，已推送main；Actions `34215486703` 成功。1号机仅替换backend，healthy/restart0、Schema063、配置修订780前后相同，DB/NanoMQ/Neuron未重启、策略启用/控制在途均0。未新增依赖、未迁移数据库、未主动发送控制。
- 现场只读首轮9项主干通过，调度懒加载超过检查默认5秒而中断；保留失败证据，只补验调度/新鲜度，按元素就绪上限45秒后通过。6个L2全部GOOD且来源时间均持续前进；页面/API错误0、业务写入0。1024/1280真实截图已检查。首次打开重型JDM仍有加载延迟，不宣称已做性能优化或像素级Demo复刻。
- 固定ARM64摘要 `4bf5da249e991fe5ca7025252d102189b18b31d997757bad13293de458ae9771`；旧镜像/配置保留，远端上传暂存包校验加载后移除。详细发布记录见 `docs/deploy-1号机-v1.0.6-http.md`。

## Session 2026-09-08 — v1.0.5 已发布到1号机

- tag `v1.0.5` 固定源码 `29a43077666b953ffc5363fe9a70e0df9b7f895b`；Actions `34211320712` 成功；main 已含验收文档，最终发布记录随本条提交。
- 1号机只替换一次backend，版本1.0.5、Schema063、healthy/restart0、配置修订777；DB/NanoMQ/Neuron未重启，无策略启用、无在途控制。固定ARM64摘要 `255f3d61ac611a01c4a407f03c6da0aa2e8f77e765497975b2b5c615dbf3cc84`。
- 新鲜062全库备份596,609,003字节；本机分阶段完整恢复及063应用重放通过。TS恢复模式导致外键验证失败已用同数据切换恢复状态验证，不删数据、不禁约束。原始失败证据、归档、两个隔离恢复库与本任务NOLOGIN角色保留，未影响现场。
- 本机121/121+18图；现场无头11项只读主干通过，业务写入/pageerror/API错误均0、采集持续。可见Browser工具两次超时，按用户要求不修工具，因此可见抽查INCOMPLETE。
- 现场仍有首页功率槽位未绑定、所选L2 INPUT_STALE，不宣称完整EMS或真实控制交付完成。最短下一步：通过现有配置绑定真实首页实体并恢复其新鲜数据；另补可见抽查。详见 `docs/deploy-1号机-v1.0.5-http.md`。
...
- 交错审查修复：槽位服务现在串行化完整的“幂等查询→栅栏→提交→重建”临界区；真实 `ConfigurationRuntimeGate` + 提交屏障回归证明，旧请求重放不能在新发布尚未提交时提前重建并打开栅栏，最终数据库与黑板均停在最新修订且失败关闭。GET 槽位契约进一步精简为 `id`、`label`、`binding_mode`、`reason`、单个可选 `entity`，移除重复的 status/source/count 字段。
- 跨入口审查修复：进程内互斥不足以覆盖 nodes/tags/L1 等共享栅栏入口，因此槽位服务不再把任意 `QUIESCED` 当成可恢复状态；仅当本服务已完成数据库提交、随后重建失败，并且重放请求与内存 pending-recovery 的 actor、幂等键、请求及回执修订完全一致时才允许恢复。真实 `nodes._apply_node_change` + `ConfigurationRuntimeGate` + 提交屏障证明，旧槽位重放会返回 BUSY，不能提前放行尚未提交的节点发布。
- TDD 证据：workbench 27/27（含真实节点入口交错）；节点发布公共 API 2/2；迁移静态契约/仓储假连接 4/4，真实 PostgreSQL 5 项因本机无 `ZIZU_POSTGRES_TEST` 测试库明确跳过；release manifest 7/7；`compileall` 通过。后端全量 757 项中 45 项仅因当前 Python 缺 `zen-engine` 报 `ZEN_ENGINE_UNAVAILABLE`，281 项条件跳过，本 seam 专项均通过。
...
- v0.7.7 已部署到 1 号机：Schema 060、healthy、restart 0、host 网络、`/dev/mqueue` tmpfs；
  TimescaleDB/NanoMQ 健康。ARM64 固定摘要为
  `sha256:c32f45d46cc3cebd2f5c7249941d527abea10c0dee7d853baeb64cf57743ad45`。
- 根因修复：告警 HTTP 通知曾把技术规则键放进 `alarm.name`；现在告警事务内冻结人类可读规则名、
  节点路径、实体名和单位，并以 `alarm.rule_key` 保留技术身份。提交 `717bb25`，v0.7.7 tag 为
  `18cda25`；现场 E2E 修订为 `78e3cf2`、`9ad4c6a`、`e4fd152`。
- 本地最终证据：PostgreSQL 告警 19/19、后端 508 通过（189 显式跳过）、发布脚本 56/56、
  前端 88/88、生产构建成功。Actions `33667644595` 成功。
- 公网无头 Browser 最终证据：告警 HTTP 3/3（164.9 秒），节点主干 7/7（306.2 秒）。已覆盖
  `节点 → L0 → L1 → L2 → 告警`，包括真实告警发生/恢复 HTTP 送达；未执行 JDM、控制或设备写。
- 已清理测试接收器及所有 E2E 在线资源。最终数据库/Neuron 检查显示活动 E2E 节点、实体、加工、
  告警、HTTP 配置、Neuron 节点及未完成通知全部为 0；近 20 分钟后端错误日志为 0。
- 完整证据：`docs/deploy-1号机-v0.7.7-http.md`。根分区可用约 2.1GB（87%），后续功能开发前应把
  磁盘监控/清理作为运维事项，但不得删除当前镜像或唯一恢复锚点。
...
- 维护者已整体确认：ZiZu 不内置邮件渠道，改为可配置 HTTP 请求通知；系统工具维护多个配置，告警规则
  可选且最多绑定一个，告警发生与现场恢复发送，人工确认不发送。
- 正式规格已写入
  `docs/superpowers/specs/2026-09-02-alarm-http-notification-design.md`。设计复用既有
  `t_alarm_notification_outbox`，提交后异步投递；2xx 成功，失败后 5 秒、30 秒、5 分钟重试，通知结果
  永不反向改变告警状态。
- HTTP 配置支持方法、URL、参数、请求头、Content-Type、请求体模板和超时；变量使用固定白名单，禁止
  任意脚本。完整 Webhook URL、敏感参数和敏感请求头均加密保存并脱敏展示。
- 维护者明确选择即时生效语义：修改配置后未完成任务下一次使用新配置；删除配置时解除规则绑定并取消
  未完成任务。规格要求逐次记录实际脱敏目标，以如实呈现同一通知可能跨地址重试的代价。
- `CONTEXT.md` 新增“HTTP 通知配置”和“告警通知任务”两个领域术语。规格完成占位符、冲突、范围和歧义
  自检，当前状态为 `pending-review`。
- 本轮只修改规格、领域词汇和交接记录，尚未编写实施计划、修改产品代码或部署；1 号机仍为 v0.7.3。
  下一步必须先由维护者复核书面规格，确认后才能调用 writing-plans 编写实施计划。
