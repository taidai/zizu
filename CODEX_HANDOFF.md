...
## Session 2026-08-15 — v0.4.79 不可变发布准备
...
- 已从最新 `origin/main` 创建 `release/0.4.79`，同步根 VERSION、后端运行版本、Python/前端包版本、lockfile、Docker 构建示例与 README 候选版本。
...
- 合并后只触发 build-only GitHub Actions；该步骤不得部署目标机、写发布锁或宣称完成交付。
- 只有取得并核验 v0.4.79 的 amd64/arm64 摘要、release.json、EMS ZIP/SHA 后，才进入目标环境部署前检查和独立交付试验。
