...
**当前代码版本：`v1.0.9`** · [English](README_EN.md) · [完整中英文架构说明](docs/ZIZU-TECHNICAL-ARCHITECTURE.md)
...
![ZiZu v1.0.9 正式首页：四指标卡、五能流节点、告警与调度摘要；数据为公开测试样例](docs/images/ems-homepage-v1.0.9.png)

> 本图截取自 v1.0.9 正式前端生产构建，不是旧 Demo。为保护现场信息，基于仓库公开测试夹具配置示例数据，API 显示 `test`；设备、数值和策略均为示例，不是现场运行证据。生产站点仍区分当前值、最后值和未配置状态，不用示例数据填充。
