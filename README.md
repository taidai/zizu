...
>
> 当前已发布候选：[**v0.4.82-rc.2**](https://github.com/taidai/zizu/releases/tag/v0.4.82-rc.2)。`v0.4.83-rc.1` 正在完成跨节点公式、全站 DAG 与机器验收门禁；取得固定镜像摘要和目标机部署证据前，不宣称生产交付就绪。
...
当前主线开发目标以[《ZiZu 产品目的地》](docs/product-destination.md)为准：物理节点树与挂载其上的 L0 原始点位、L1 点位加工、L2 全局实体共同构成平台数据主干；告警、策略、控制、画面和报表只消费 L2 稳定业务语义。

```
协议设备 → L0 原始点位 → L1 点位加工 → L2 全局实体 → EMS 运行、历史、告警、策略与控制
```
...
下表描述现有代码形成的功能域和迁移基线，不再作为目标领域分层。后续实现按节点树及 L0—L2 数据主干逐步收口，旧 F0/F1/F2/F3/F4 编号仅用于追踪既有代码和兼容入口。
