...
**当前版本：`v0.9.8`** · [English](README_EN.md) · [完整中英文架构说明](docs/ZIZU-TECHNICAL-ARCHITECTURE.md)

[v0.9.8 发布与验收记录](docs/deploy-1号机-v0.9.8-http.md) · [上轮现网功能评估](docs/reviews/2026-09-06-v0.9.5-live-readiness.md)

本版已部署：健康检查不再阻塞采集事件循环，数据帧直接按索引读取持久化观测；沿用单次下发、失败停用及原期限内回读确认。真实 PCS 的只读主干页面复验通过。短窗中 MQTT 到提交值首次可见的中位延迟由约 3.8 秒降至 2.9 秒，但仍出现 5.6 秒延迟，另四次当前值采样全部 STALE，真实控制时效仍未通过验收。本轮未写设备，不放宽安全门，页面可用不等于完整 EMS 已交付。
