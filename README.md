...
**当前代码版本：`v1.0.2`** · [English](README_EN.md) · [完整中英文架构说明](docs/ZIZU-TECHNICAL-ARCHITECTURE.md)

[v0.9.9 发布与验收记录](docs/deploy-1号机-v0.9.9-http.md) · [上轮现网功能评估](docs/reviews/2026-09-06-v0.9.5-live-readiness.md)

最新代码包含调度策略数据库连接复用修复，并在策略卡片中区分“当前 L2”和“决策时值”，避免把决策快照误当成设备回读。上方历史部署与验收记录仅适用于对应版本，不代表当前所有功能均已完成现场验收。
...
> 这是设计样稿，**并非现网截图，该页面尚未部署**。图中的设备、拓扑、功率、SOC、告警、调度与能流数据全部为演示数据，不是实际运行或验收证据。运行页面与工程配置的分组不改变原有权限和控制安全边界。
...
- 配置告警和调度策略时，分别在界面中选择同一个 L2 实体身份；两者只消费 committed L2，不能以页面可见或品牌点位替代身份、质量和来源证据核对。
...
- 现场配置更新后，点击“保存草稿”可重新核对当前实体并更新草稿的配置基线；旧绑定未填的单位可随当前实体补齐，已知单位或类型发生变化仍会阻断。原发布版本和启用状态不变，必须重新试算，再显式发布与启用。数据超时仍停止试算和控制，不自动延长新鲜度期限。
- 对可控 L2 配置唯一写点、限值、联锁、权限、超时和回读条件。
- 每个调度意图只下发一次；失败后人工处理并显式重新启用，清除失败记录不会自动恢复控制。回读始终受原截止时间及数据质量约束，接口返回成功不代表设备已到位。
