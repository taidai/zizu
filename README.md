...
前端“解决方案交付”页提供同一条受保护流程：管理员上传并选择已验证的包，按
`parameter_contracts` 填写站点参数和 `secret://` 引用，审查计划中的变更及 blocker 后确认
安装；安装记录可在页内运行机器验收并读取不可变报告。页面不保存 Secret 明文，也不会自行
下发策略或控制命令。当前包目录仍仅管理员可读取：工程师能够执行其获授权的安装/验收 API，
但要以管理员签发的包记录和参数工作单为输入；独立“安装工作单”是后续交付流程需要补齐的
产品对象。
