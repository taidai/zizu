...
**当前版本：`v0.9.7`** · [English](README_EN.md) · [完整中英文架构说明](docs/ZIZU-TECHNICAL-ARCHITECTURE.md)

[v0.9.7 发布与验收记录](docs/deploy-1号机-v0.9.7-http.md) · [上轮现网功能评估](docs/reviews/2026-09-06-v0.9.5-live-readiness.md)

本版已部署：回读在原期限内等待有效匹配，不因首条不匹配提前失败；调度意图只下发一次，失败停用并锁定策略；数据来源改为批量保存。真实 PCS 的只读主干页面复验通过，但通信阈值点位的现场数据仍出现 5～12.5 秒龄并被判 STALE，真实控制闭环尚未通过验收。本轮未写设备，页面可用不等于控制时效达标。
