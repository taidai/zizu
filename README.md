...
**当前版本：`v0.9.9`** · [English](README_EN.md) · [完整中英文架构说明](docs/ZIZU-TECHNICAL-ARCHITECTURE.md)

[v0.9.9 发布与验收记录](docs/deploy-1号机-v0.9.9-http.md) · [上轮现网功能评估](docs/reviews/2026-09-06-v0.9.5-live-readiness.md)

本版已部署：补齐固定节拍 JDM 的本机真实后端无头纵向验收，浏览器、FastAPI、PostgreSQL/TimescaleDB、标准 GoRules JDM、数据帧 outbox、统一控制与 committed L2 回读均真实参与，只替代设备通信边界；成功、同分钟防重、失败停用和停用后不再触发均有自动化证据。真实 PCS 的节点树、L0、L1、L2、告警和 JDM 只读主干复验通过，但选定 L2 当次仍为过期数据，真实控制闭环尚未验收。本轮未写设备、不启用策略、不放宽安全门。
