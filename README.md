...
当前主线开发目标以[《ZiZu 产品目的地》](docs/product-destination.md)为准：物理节点树与挂载其上的 L0 原始点位、L1 点位转换、L2 全局实体共同构成平台数据主干；告警、策略、控制、画面和报表只消费 L2 稳定业务语义。

```
协议设备 → L0 原始点位 → L1 点位转换 → L2 全局实体 → EMS 运行、历史、告警、策略与控制
```

> **节点树负责组织现场，点位转换负责消除品牌差异，全局实体负责向上提供稳定业务语义。**
...
这是一项已实现的历史结构。目标结构中的节点树只表达站点、能源子系统和设备的真实物理拓扑；Tag 不再伪装成物理树层级，L0 原始点位、L1 点位转换和 L2 全局实体改为所选物理节点的三个数据视角。
