            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    SELECT entity.definition_id, entity.source_kind,
                           ...
                           binding.estimated, installed.raw_detail_retention_days,
                           audit.action
    ...
    def test_window_result_lifecycle_and_l2_reference_checks_are_enforced(self) -> None:
        ...
        statement = """
            INSERT INTO t_business_metric_window_results
              (installed_metric_id, window_started_at, window_ended_at,
               revision, lifecycle, quality, coverage, estimated,
               source_count, result_event_id, result_observed_at,
               content_digest, source_summary)
            VALUES (%s, now() - interval '1 day', now(), 1, %s, 192,
                    1, FALSE, 1, %s, %s, %s, '{}')
        """
        cases = (
            (
                psycopg2.errors.CheckViolation,
                (installed.id, "completed", None, None, "f" * 64),
            ),
            (
                psycopg2.errors.CheckViolation,
                (installed.id, "invalid", uuid4(), None, "f" * 64),
            ),
            (
        ...
        with psycopg2.connect(**self.connection_kwargs) as connection:
            with connection.cursor() as cursor:
                ...
                loaded = PostgresPointProcessingCatalog._load_asset(
                    cursor, compiled.processing_revision_id
                )
