...
export default function RuleSetEditor({ rules, onChange, disabled }: Props) {
  ...
  return (
    <section aria-labelledby="alarm-rules-heading" className="neu-card p-4">
      ...
      <div className="mt-3 space-y-3 md:hidden">{rules.map((rule, index) => <article key={index} className="rounded border border-white/70 p-3 text-xs"><div className="mb-2 flex items-center justify-between"><strong>规则 {index + 1}</strong><button type="button" onClick={() => onChange(rules.filter((_, itemIndex) => itemIndex !== index))} disabled={disabled} className="rounded px-2 py-1 text-red-600 disabled:opacity-50">移除</button></div><div className="grid gap-2"><label>标识<input aria-label={`规则 ${index + 1} 标识`} value={rule.id} onChange={(event) => patchRule(index, { id: event.target.value })} disabled={disabled} className="neu-input mt-1 w-full px-2 py-1.5" /></label><label>名称<input aria-label={`规则 ${index + 1} 名称`} value={rule.name} onChange={(event) => patchRule(index, { name: event.target.value })} disabled={disabled} className="neu-input mt-1 w-full px-2 py-1.5" /></label><label>严重度<select aria-label={`规则 ${index + 1} 严重度`} value={rule.severity} onChange={(event) => patchRule(index, { severity: event.target.value as AlarmSeverity })} disabled={disabled} className="neu-input mt-1 w-full px-2 py-1.5">{SEVERITIES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><div className="grid grid-cols-2 gap-2"><label>触发条件<select aria-label={`规则 ${index + 1} 触发运算符`} value={rule.trigger.operator} onChange={(event) => patchCondition(index, 'trigger', { operator: event.target.value as AlarmConditionOperator })} disabled={disabled} className="neu-input mt-1 w-full px-2 py-1.5">{OPERATORS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><input aria-label={`规则 ${index + 1} 触发值`} type="number" value={rule.trigger.value} onChange={(event) => patchCondition(index, 'trigger', { value: Number(event.target.value) })} disabled={disabled} className="neu-input mt-1 w-full px-2 py-1.5" /></label><label>恢复条件<select aria-label={`规则 ${index + 1} 恢复运算符`} value={rule.recovery.operator} onChange={(event) => patchCondition(index, 'recovery', { operator: event.target.value as AlarmConditionOperator })} disabled={disabled} className="neu-input mt-1 w-full px-2 py-1.5">{OPERATORS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><input aria-label={`规则 ${index + 1} 恢复值`} type="number" value={rule.recovery.value} onChange={(event) => patchCondition(index, 'recovery', { value: Number(event.target.value) })} disabled={disabled} className="neu-input mt-1 w-full px-2 py-1.5" /></label></div><div className="grid grid-cols-3 gap-2"><label>触发持续<input aria-label={`规则 ${index + 1} 触发持续秒数`} type="number" min="0" value={rule.trigger_duration_seconds} onChange={(event) => patchRule(index, { trigger_duration_seconds: Number(event.target.value) })} disabled={disabled} className="neu-input mt-1 w-full px-2 py-1.5" /></label><label>恢复持续<input aria-label={`规则 ${index + 1} 恢复持续秒数`} type="number" min="0" value={rule.recovery_duration_seconds} onChange={(event) => patchRule(index, { recovery_duration_seconds: Number(event.target.value) })} disabled={disabled} className="neu-input mt-1 w-full px-2 py-1.5" /></label><label>通知间隔<input aria-label={`规则 ${index + 1} 通知间隔秒数`} type="number" min="0" value={rule.notification_throttle_seconds} onChange={(event) => patchRule(index, { notification_throttle_seconds: Number(event.target.value) })} disabled={disabled} className="neu-input mt-1 w-full px-2 py-1.5" /></label></div></div></article>)}{rules.length === 0 && <p className="py-6 text-center text-gray-400">添加至少一条规则后才能生成计划。</p>}</div>
