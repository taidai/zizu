...
export default function PointConversionPlanPanel({
  trunk,
  templates,
  selectedTemplate,
  selections,
  ...
  trunk: NodeDataTrunk
  templates: PointConversionTemplate[]
  selectedTemplate: PointConversionTemplate | null
  selections: Record<string, string>
  plan: PointConversionPlan | null
    ...
    <aside className="rounded-xl border border-gray-200 bg-white/55 p-4">
      ...
      {plan && (
        <div className="mt-4 border-t border-gray-200 pt-4">
          ...
          >
            {busy === 'apply' ? '正在应用...' : resultUnknown ? '使用原请求重试' : '应用点位转换'}
          </button>
        </div>
      )}
