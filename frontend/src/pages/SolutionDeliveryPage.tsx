...
export default function SolutionDeliveryPage({ canImport }: Props) {
  ...
  const runAcceptance = async (installation: SolutionInstallation) => {
    try {
      const parsed = JSON.parse(policyCommands) as Record<string, string>
      const next = await runDeliveryAcceptance(installation.id, parsed)
      setReport(next)
