...
export default function SolutionDeliveryPage({ canImport }: Props) {
  ...
  const [authorizationDenials, setAuthorizationDenials] = useState('{}')
  ...
  const createPlan = async () => {
    ...
    try {
      ...
      const next = await createInstallationPlan(selected.id, {
        parameters,
        secret_references,
  ...
  const overrideItems = (plan?.items || []).filter(
    (item): item is EntityBindingPlanItem =>
      item.kind === 'entity_binding'
      && Array.isArray(item.override_candidates)
      && item.override_candidates.length > 0,
  )
  ...
  const runAcceptance = async (installation: SolutionInstallation) => {
    try {
      ...
      const denials = JSON.parse(authorizationDenials) as Record<string, string>
  ...
  return <section className="space-y-4">
    ...
    {canImport ? <div className="neu-card p-5"><label className="block text-sm font-medium text-gray-700">导入已签收的解决方案包<input type="file" accept=".zip,.zizu.zip,application/zip" onChange={(event) => void upload(event)} className="mt-2 block w-full text-xs text-gray-600" /></label></div> : <div className="neu-card p-5 text-sm text-gray-500">可从已验证解决方案包开始计划、安装和验收；解决方案包导入与生命周期管理由平台管理员完成。</div>}
    {selected && <div className="neu-card p-5"><div className="flex flex-wrap items-center justify-between gap-3"><label className="text-sm font-medium text-gray-700">解决方案<select value={selectedPackageId} onChange={(event) => { setSelectedPackageId(event.target.value); setPlan(null); setBindingOverrides({}) }} className="neu-input ml-2 px-2 py-1.5 text-sm">{packages.map((item) => <option value={item.id} key={item.id}>{item.display_name} · {item.version}</option>)}</select></label><span className="font-mono-value text-[10px] text-gray-400">{selected.digest.slice(0, 16)}…</span></div><div className="mt-4 grid gap-3 md:grid-cols-2">{selected.parameter_contracts.map((contract) => <label key={contract.id} className="block text-xs font-medium text-gray-600">{contract.id}{contract.required && ' *'}<span className="ml-1 font-normal text-gray-400">{contract.description || contract.type}</span>{contract.type === 'boolean' ? <select value={values[contract.id] || ''} onChange={(event) => setValues({ ...values, [contract.id]: event.target.value })} className="neu-input mt-1 block w-full px-3 py-2 text-sm"><option value="">选择</option><option value="true">true</option><option value="false">false</option></select> : <textarea value={values[contract.id] || ''} onChange={(event) => setValues({ ...values, [contract.id]: event.target.value })} placeholder={contract.type === 'device_instances' ? '[{"instance_key":"PCS-01","device_key":"PCS-01"}]' : contract.type === 'secret' ? 'secret://approved-reference' : contract.type} className="neu-input mt-1 block min-h-10 w-full px-3 py-2 text-sm" />}</label>)}</div><button onClick={() => void createPlan()} className="mt-4 rounded-lg bg-[#52c41a] px-4 py-2 text-sm font-medium text-white">{plan && Object.keys(bindingOverrides).length > 0 ? '按所选标签重新规划' : '生成安装计划'}</button></div>}
    {plan && <div className="neu-card p-5"><h3 className="font-semibold text-gray-700">安装计划</h3><p className="mt-1 text-xs text-gray-400">{plan.status} · {plan.id}</p>{plan.blockers.length > 0 && <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-red-700">{plan.blockers.map((item, index) => <li key={`${item.code}-${index}`}>{item.code}: {item.message}</li>)}</ul>}{overrideItems.length > 0 && <div className="mt-4 space-y-3 rounded-lg border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-semibold text-amber-900">现场标签名称不同，请由实施工程师确认映射</p>{overrideItems.map((item) => <label key={item.asset_id} className="block text-xs font-medium text-amber-950"><span className="font-mono">{item.expected_tag_name}</span> → 现场标签<select value={bindingOverrides[item.asset_id] || ''} onChange={(event) => setBindingOverrides((current) => ({ ...current, [item.asset_id]: event.target.value }))} className="neu-input mt-1 block w-full px-3 py-2 text-sm"><option value="">请选择兼容标签</option>{item.override_candidates.map((candidate) => <option value={candidate.tag_id} key={candidate.tag_id}>{candidate.device_name} / {candidate.tag_name} · {candidate.data_type}{candidate.unit ? ` ${candidate.unit}` : ''} · {candidate.direction}</option>)}</select></label>)}</div>}<div className="mt-3 max-h-64 overflow-auto rounded-lg bg-gray-50 p-3 font-mono text-[11px] text-gray-600">{plan.items.map((item, index) => <pre key={index}>{JSON.stringify(item, null, 2)}</pre>)}</div><button disabled={plan.blockers.length > 0} onClick={() => void apply()} className="mt-4 rounded-lg bg-[#52c41a] px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50">确认安装</button></div>}
    <div className="neu-card p-5"><h3 className="font-semibold text-gray-700">已安装解决方案与验收</h3><label className="mt-3 block text-xs font-medium text-gray-600">手动控制命令映射（仅包要求手动控制回读验收时填写）<textarea value={manualCommands} onChange={(event) => setManualCommands(event.target.value)} className="neu-input mt-1 block min-h-10 w-full px-3 py-2 text-sm" /></label><label className="mt-3 block text-xs font-medium text-gray-600">策略命令映射（仅包要求策略执行验收时填写）<textarea value={policyCommands} onChange={(event) => setPolicyCommands(event.target.value)} className="neu-input mt-1 block min-h-10 w-full px-3 py-2 text-sm" /></label><label className="mt-3 block text-xs font-medium text-gray-600">权限拒绝审计映射（以低权限账号请求受限操作后，将响应头 X-ZiZu-Audit-Event-ID 填入）<textarea value={authorizationDenials} onChange={(event) => setAuthorizationDenials(event.target.value)} className="neu-input mt-1 block min-h-10 w-full px-3 py-2 text-sm" /></label><div className="mt-3 space-y-2">{installations.map((installation) => <div key={installation.id} className="rounded-lg border border-white/60 bg-white/30 p-3 text-sm"><span>{installation.id} · 配置版本 {installation.site_configuration_version}</span><button onClick={() => void runAcceptance(installation)} className="ml-3 rounded-lg border border-gray-300 px-3 py-1 text-xs text-gray-700">运行验收</button></div>)}{installations.length === 0 && <p className="text-sm text-gray-400">尚无安装记录。</p>}</div>{report && <div className="mt-4 rounded-lg bg-gray-50 p-3"><p className="text-sm font-semibold text-gray-700">报告 {report.status}</p>{report.items.map((item) => <p className="mt-1 text-xs text-gray-600" key={item.acceptance_id}>{item.acceptance_id}: {item.status} · {item.code}</p>)}</div>}</div>
