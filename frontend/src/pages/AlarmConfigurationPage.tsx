...
export default function AlarmConfigurationPage() {
  ...
  const applyPlan = async () => { if (!plan) return; const key = idempotencyKey || crypto.randomUUID(); saveApplyContext(plan, key); setIdempotencyKey(key); setApplying(true); setError(''); setSuccess(''); try { const result = await applyAlarmConfigurationPlan(plan.id, plan.digest, key); setSuccess(`已应用告警配置，站点配置版本已更新至 ${result.site_configuration_version}。`); resetPlan(); await load() } catch (reason) { if (stalePlan(reason)) markPlanStale(); else if (unrecoverableReplay(reason)) resetPlan(); else if (definitiveApplyFailure(reason)) clearKnownApplyEvidence(); setError(alarmWorkspaceError(reason, '应用结果无法确认，请使用同一操作重试。')) } finally { setApplying(false) } }
