...
export default function AlarmConfigurationPage() {
  ...
  const migrate = async () => {
    const selections = migrationCandidates.filter((candidate) => candidate.blockers.some((blocker) => blocker.code === 'ALARM_MIGRATION_AMBIGUOUS')).map((candidate) => ({ source_kind: candidate.source_kind, source_key: candidate.source_key, entity_instance_id: migrationSelections[`${candidate.source_kind}:${candidate.source_key}`] })).filter((selection) => selection.entity_instance_id)
