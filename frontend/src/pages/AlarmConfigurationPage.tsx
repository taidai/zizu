...
import { AlarmConfigurationApiError, AlarmConfigurationResultUnknownError, applyAlarmConfigurationPlan, createAlarmConfigurationPlan, createAlarmRuleSet, createAlarmRuleSetRevision, createLegacyAlarmMigrationPlan, fetchAlarmRuleSets, fetchEntityInstances, fetchLegacyAlarmMigrationCandidates, getAlarmConfigurationPlan, getUnifiedAlarmConfiguration, type AlarmConfigurationCurrent, type AlarmConfigurationPlan, type AlarmRule, type AlarmRuleSetRevision, type EntityInstance, type LegacyAlarmMigrationCandidate } from '../api/client'
...
export default function AlarmConfigurationPage() {
  ...
  const selectedRevisionRef = useRef<AlarmRuleSetRevision | null>(null); const initialContextRef = useRef(readWorkspaceContext()); const restoredPlanRef = useRef('')
    ...
    void getAlarmConfigurationPlan(context.plan.id).then(async (loaded) => {
      ...
      if (replay && context.apply) {
        ...
        try { const result = await applyAlarmConfigurationPlan(loaded.id, context.apply.digest, context.apply.key); setSuccess(`已恢复并确认告警配置，站点配置版本已更新至 ${result.site_configuration_version}。`); resetPlan(); await load() }
          ...
          else if (!(reason instanceof AlarmConfigurationResultUnknownError)) clearKnownApplyEvidence()
  ...
  const applyPlan = async () => { if (!plan) return; const key = idempotencyKey || crypto.randomUUID(); saveApplyContext(plan, key); setIdempotencyKey(key); setApplying(true); setError(''); setSuccess(''); try { const result = await applyAlarmConfigurationPlan(plan.id, plan.digest, key); setSuccess(`已应用告警配置，站点配置版本已更新至 ${result.site_configuration_version}。`); resetPlan(); await load() } catch (reason) { if (stalePlan(reason)) markPlanStale(); else if (unrecoverableReplay(reason)) resetPlan(); else if (!(reason instanceof AlarmConfigurationResultUnknownError)) clearKnownApplyEvidence(); setError(alarmWorkspaceError(reason, '应用结果无法确认，请使用同一操作重试。')) } finally { setApplying(false) } }
  ...
  return <div className="space-y-4"><header className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-base font-bold text-gray-800">统一告警配置</h2><p className="mt-1 text-xs text-gray-500">面向实施工程师的预览优先工作台。严重度固定为严重、重要、警告、提示。</p></div><div className="flex items-center gap-3 text-xs text-gray-500"><span>当前配置版本 {current?.site_configuration_version ?? '未加载'}</span><span>旧配置候选 {migrationCandidates.length}</span><button type="button" onClick={() => void load()} className="neu-btn px-3 py-1.5 text-xs" disabled={loading}>刷新</button></div></header>{error && <div role="alert" className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-800">{error}</div>}{success && <div role="status" className="rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-xs text-green-800">{success}</div>}{planStale && !plan && <div role="status" className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">原计划已过期并清除，请人工重新生成变更预览。</div>}<section aria-labelledby="current-alarm-definitions" className="neu-card p-4"><h3 id="current-alarm-definitions" className="text-sm font-bold text-gray-800">当前告警定义</h3><div className="mt-2 grid gap-2 md:grid-cols-2">{current?.definitions.map((definition, index) => <article key={`${definition.entity_display_name}-${definition.rule_name}-${index}`} className="rounded border border-white/70 p-2 text-xs"><strong>{definition.entity_display_name}</strong><p className="mt-1 text-gray-600">{definition.rule_name}，{severityLabel[definition.severity]}，触发 {operatorLabel[definition.trigger.operator]} {definition.trigger.value}，恢复 {operatorLabel[definition.recovery.operator]} {definition.recovery.value}</p><p className="mt-1 text-gray-500">{definition.version_description}　{definition.enabled ? '已启用' : '未启用'}　当前</p></article>)}{!current?.definitions.length && <p className="text-xs text-gray-400">当前没有已安装的告警定义。</p>}</div></section><section aria-labelledby="rule-set-heading" className="neu-card p-4"><div className="grid gap-3 md:grid-cols-2"><label className="text-xs font-medium text-gray-700">使用已有规则集<select aria-label="选择已有规则集" value={selectedRuleSetKey} onChange={(event) => chooseRuleSet(event.target.value)} className="neu-input mt-1 block w-full px-3 py-2 text-xs"><option value="">新建规则集</option>{ruleSets.map((item) => <option key={revisionKey(item)} value={revisionKey(item)}>{item.name}（第 {item.revision} 版）</option>)}</select></label>{!selectedRuleSetKey ? <label className="text-xs font-medium text-gray-700">新规则集名称<input value={newRuleSetName} onChange={(event) => setNewRuleSetName(event.target.value)} className="neu-input mt-1 block w-full px-3 py-2 text-xs" maxLength={200} /></label> : <p className="self-end pb-2 text-xs text-gray-500">修改规则后会在生成计划时创建新的修订，不会覆盖既有版本。</p>}</div></section><EntityScopePicker entities={entities} value={scope} onChange={(next) => { setScope(next); resetPlan() }} disabled={planning || applying} /><RuleSetEditor rules={rules} onChange={(next) => { setRules(next); resetPlan() }} disabled={planning || applying} /><div className="flex justify-end"><button type="button" onClick={() => void createPlan()} disabled={planning || applying || !scopeHasValue} className="rounded-lg bg-[#52c41a] px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-[#389e0d] focus:outline-none focus:ring-2 focus:ring-[#52c41a]/60 disabled:cursor-not-allowed disabled:opacity-50">{planning ? '正在生成计划...' : '生成变更预览'}</button></div>{plan && <PlanPreview plan={plan} entityNames={entityNames} applying={applying} stale={planStale} onApply={() => void applyPlan()} />}<LegacyMigrationPanel candidates={migrationCandidates} entities={entities} selections={migrationSelections} onSelectionChange={(sourceKind, sourceKey, entityInstanceId) => setMigrationSelections((previous) => ({ ...previous, [`${sourceKind}:${sourceKey}`]: entityInstanceId }))} onMigrate={() => void migrate()} migrating={migrating} /></div>
}
