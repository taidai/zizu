...
export default function AlarmConfigurationPage({ actorId, onOpenAlarms }: { actorId: string; onOpenAlarms: () => void }) {
  ...
  useEffect(() => {
    const context = readWorkspaceContext(); const matchesWorkspace = context.plan?.revision === 'legacy_migration' || context.plan?.revision === selectedRuleSetKey; if (!current || !context.plan || !matchesWorkspace || restoredPlanRef.current === context.plan.id) return
  ...
  const migrate = async () => { const selections = migrationCandidates.filter((candidate) => candidate.blockers.some((blocker) => blocker.code === 'ALARM_MIGRATION_AMBIGUOUS')).map((candidate) => ({ source_kind: candidate.source_kind, source_key: candidate.source_key, entity_instance_id: migrationSelections[`${candidate.source_kind}:${candidate.source_key}`] })).filter((selection) => selection.entity_instance_id); setMigrating(true); setError(''); setSuccess(''); try { const created = await createLegacyAlarmMigrationPlan({ installation_id: migrationInstallationId, selections }); setPlan(created); savePlanContext(created); initialContextRef.current = readWorkspaceContext(); setPlanStale(false); setIdempotencyKey(null); setSuccess('旧配置迁移计划已生成，请在变更预览中确认后应用。') } catch (reason) { setError(alarmWorkspaceError(reason, '旧配置迁移计划生成失败，请检查候选后重试。')) } finally { setMigrating(false) } }
