...
function AcceptancePanel({ progress, report, entityNames, loading, running, onRefresh, onGenerate, onOpenAlarms }: {
  ...
  return <section aria-labelledby="alarm-acceptance-heading" className="neu-card p-4">
    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
      ...
      <div className="flex flex-col gap-2 sm:flex-row md:justify-end"><button type="button" onClick={onRefresh} disabled={loading || running} className="neu-btn px-3 py-2 text-xs disabled:cursor-not-allowed disabled:opacity-50">{loading ? '正在读取证据...' : '刷新证据'}</button><button type="button" onClick={onGenerate} disabled={!progress?.ready_to_report || loading || running} className="rounded-lg bg-[#52c41a] px-3 py-2 text-xs font-semibold text-white shadow-sm hover:bg-[#389e0d] focus:outline-none focus:ring-2 focus:ring-[#52c41a]/60 disabled:cursor-not-allowed disabled:opacity-50">{running ? '正在生成报告...' : '生成验收报告'}</button></div>
    </div>
...
export default function AlarmConfigurationPage({ onOpenAlarms }: { onOpenAlarms: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [configuration, revisions, entityResponse, migrations, progress] = await Promise.all([getUnifiedAlarmConfiguration(), fetchAlarmRuleSets(), fetchEntityInstances(), fetchLegacyAlarmMigrationCandidates(), progressOrEmpty()])
  ...
  const refreshAcceptance = async () => { setAcceptanceLoading(true); setError(''); try { const progress = await progressOrEmpty(); if (acceptanceApplicationRef.current && acceptanceApplicationRef.current !== progress?.application_id) { setAcceptanceReport(null); setAcceptanceIdempotencyKey(null) }; acceptanceApplicationRef.current = progress?.application_id || ''; setAcceptanceProgress(progress) } catch (reason) { setError(alarmWorkspaceError(reason, '无法刷新验收证据，请稍后重试。')) } finally { setAcceptanceLoading(false) } }
  const runAcceptance = async () => { if (!acceptanceProgress?.ready_to_report) return; const key = acceptanceIdempotencyKey || crypto.randomUUID(); setAcceptanceIdempotencyKey(key); setAcceptanceRunning(true); setError(''); setSuccess(''); try { const created = await runAlarmConfigurationAcceptance(acceptanceProgress.application_id, key); const report = await fetchAlarmConfigurationReport(created.id); setAcceptanceReport(report); setSuccess('不可变验收报告已生成并重新读取确认。') } catch (reason) { setError(alarmWorkspaceError(reason, '验收报告结果无法确认，请使用同一操作重试。')) } finally { setAcceptanceRunning(false) } }
  ...
  return <div className="space-y-4">
    <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
      ...
      <div className="flex flex-col gap-2 text-xs text-gray-500 sm:flex-row sm:items-center"><span>当前配置版本 {current?.site_configuration_version ?? '未加载'}</span><span>旧配置候选 {migrationCandidates.length}</span><button type="button" onClick={() => void load()} className="neu-btn px-3 py-1.5 text-xs" disabled={loading}>刷新</button></div>
    </header>
    ...
    <AcceptancePanel progress={acceptanceProgress} report={acceptanceReport} entityNames={entityNames} loading={acceptanceLoading} running={acceptanceRunning} onRefresh={() => void refreshAcceptance()} onGenerate={() => void runAcceptance()} onOpenAlarms={onOpenAlarms} />
