...
const entities = {
  site: {
    ...
    direction: 'R', status: 'available', value: 0, quality: 192, observed_at: '2026-09-08T08:00:00Z',
  },
  pv: {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (url.pathname.endsWith('/entity-instances')) return fulfillJson(route, {
      items: Object.values(entities).map((item) => ({
        ...
        unit: item.unit, direction: item.direction, freshness_seconds: 60, confirmed: true,
      })), total: 3,
    })
    ...
    if (url.pathname.endsWith('/ems-workbench')) return fulfillJson(route, {
      workbench_id: 'fixed-light-storage-charging', configuration_revision: 12,
      ...
      kpis: [
        ...
        { id: 'storage-power', label: '储能功率', binding_mode: 'ambiguous', reason: '找到 2 个精确候选，需人工选择', entity: null },
...
for (const viewport of [{ width: 1024, height: 768 }, { width: 1280, height: 800 }]) {
  test(`formal overview composes five truthful slots at ${viewport.width}x${viewport.height}`, async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('region', { name: '站点能流' })).toContainText(/接口未提供.*方向语义/)
...
test('metric opens the existing committed-L2 detail instead of exposing frame evidence on the overview', async ({ page }) => {
  ...
  await expect(page.getByRole('main')).not.toContainText('sha256:')
})
...
test('engineer can bind and clear a fixed slot with current revision and stable idempotency headers', async ({ page }) => {
  ...
  await page.route('**/api/v1/ems-workbench/slots/storage-power', async (route) => {
    ...
    await fulfillJson(route, { slot_key: 'storage-power', entity_instance_id: calls.length === 1 ? 'entity-pv' : null, configuration_revision: 12 + calls.length, replayed: false })
  })
  await page.goto('/', { waitUntil: 'networkidle' })
  ...
  await dialog.getByRole('button', { name: '清除储能功率绑定' }).click()
  await expect(dialog.getByRole('status')).toContainText('配置修订 14')
