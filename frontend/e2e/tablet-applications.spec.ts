...
for (const viewport of [{ width: 1280, height: 800 }, { width: 1024, height: 768 }]) {
  test(`告警、调度和系统工具在 ${viewport.width}x${viewport.height} 保持触控可达且只读打开`, async ({ page }) => {
    ...
    await expectTouchTargets(page, ['打开NanoMQ / MQTT', '打开HTTP 通知', '打开故障映射', '打开数据与系统状态'])
    await tools.getByRole('button', { name: '打开NanoMQ / MQTT', exact: true }).click()
    const messaging = page.getByRole('dialog', { name: 'NanoMQ / MQTT' })
    await expectTouchTargets(page, ['保存配置', '保存并重订阅'])
    await messaging.getByRole('button', { name: '关闭', exact: true }).click()
    await tools.getByRole('button', { name: '打开数据与系统状态', exact: true }).click()
    const dataTools = page.getByRole('dialog', { name: '数据与系统状态' })
    const systemHealth = dataTools.getByRole('region', { name: '系统健康状态' })
    await expect(systemHealth).toContainText('healthy')
    await expect(systemHealth).toContainText('TimescaleDB connected')
    await expect(systemHealth).toContainText('MQTT connected')
    await expect(dataTools.getByRole('button', { name: '执行', exact: true })).toBeVisible()
    await expect(dataTools.getByRole('button', { name: '准备清空表', exact: true })).toBeVisible()
    await page.waitForTimeout(1_000)
    expect(runtimeErrors).toEqual([])
