...
async function installToolsApi(page: Page, role: 'admin' | 'engineer' = 'admin') {
  const writes: string[] = []
...
test('系统工具以四个正式分区打开管理器且打开动作不产生写请求', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog', { name: '数据与系统状态' }).getByRole('button', { name: '执行', exact: true })).toBeVisible()
