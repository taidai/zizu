...
test('系统工具顶层、嵌套编辑器和永久确认框形成键盘焦点闭环', async ({ page }) => {
  await installToolsApi(page)
  await login(page)
  await openEngineeringPage(page, '系统工具')

  const openHttp = page.getByRole('button', { name: '打开HTTP 通知', exact: true })
  await openHttp.click()
  const httpParent = page.getByRole('dialog', { name: 'HTTP 通知', exact: true })
  const httpClose = httpParent.getByRole('button', { name: '关闭', exact: true })
  const httpCard = httpParent.locator('article').filter({ hasText: '值班系统' })
  const deleteHttp = httpCard.getByRole('button', { name: '删除', exact: true })

  await expect(httpClose).toBeFocused()
  await expect(deleteHttp).toBeVisible()
  await page.keyboard.press('Shift+Tab')
  await expect(deleteHttp).toBeFocused()
  await page.keyboard.press('Tab')
  await expect(httpClose).toBeFocused()
  await page.keyboard.press('Escape')
  await expect(httpParent).toHaveCount(0)
  await expect(openHttp).toBeFocused()

  await openHttp.click()
  const editHttp = httpCard.getByRole('button', { name: '编辑', exact: true })
  await editHttp.click()
  const httpEditor = page.getByRole('dialog', { name: 'HTTP 通知编辑器', exact: true })
  const httpEditorClose = httpEditor.getByRole('button', { name: '关闭', exact: true })
  const saveHttp = httpEditor.getByRole('button', { name: '保存', exact: true })
  ...
  await expect(editHttp).toBeFocused()
  await editHttp.click()
  await page.getByRole('dialog', { name: 'HTTP 通知编辑器', exact: true }).getByRole('button', { name: '关闭', exact: true }).click()
  await expect(editHttp).toBeFocused()
  await page.keyboard.press('Escape')
  await expect(openHttp).toBeFocused()

  const openFaults = page.getByRole('button', { name: '打开故障映射', exact: true })
  await openFaults.click()
  const faultParent = page.getByRole('dialog', { name: '故障映射', exact: true })
  const createFaultMap = faultParent.getByRole('button', { name: '新建映射表', exact: true })
  await createFaultMap.click()
  const faultEditor = page.getByRole('dialog', { name: '故障映射编辑器', exact: true })
  const faultEditorClose = faultEditor.getByRole('button', { name: '关闭', exact: true })
  const saveFaultMap = faultEditor.getByRole('button', { name: '保存', exact: true })
  await expect(faultEditorClose).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(saveFaultMap).toBeFocused()
  await page.keyboard.press('Tab')
  await expect(faultEditorClose).toBeFocused()
  await page.keyboard.press('Escape')
  await expect(faultEditor).toHaveCount(0)
  await expect(faultParent).toBeVisible()
  await expect(createFaultMap).toBeFocused()
  await createFaultMap.click()
  await page.getByRole('dialog', { name: '故障映射编辑器', exact: true }).getByRole('button', { name: '取消', exact: true }).click()
  await expect(createFaultMap).toBeFocused()
  await page.keyboard.press('Escape')
  await expect(openFaults).toBeFocused()

  const openData = page.getByRole('button', { name: '打开数据与系统状态', exact: true })
  await openData.click()
  const dataParent = page.getByRole('dialog', { name: '数据与系统状态', exact: true })
  const prepare = dataParent.getByRole('button', { name: '准备清空表', exact: true })
  await prepare.click()
  const confirm = page.getByRole('dialog', { name: '确认永久清空数据', exact: true })
  const input = confirm.getByLabel('输入 yes 确认')
  const permanent = confirm.getByRole('button', { name: '永久清空', exact: true })
  ...
  await expect(dataParent).toBeVisible()
  await expect(prepare).toBeFocused()
  await prepare.click()
  await page.getByRole('dialog', { name: '确认永久清空数据', exact: true }).getByRole('button', { name: '取消', exact: true }).click()
  await expect(dataParent).toBeVisible()
  await expect(prepare).toBeFocused()
})

test('异步系统健康响应完成后不抢走当前输入焦点', async ({ page }) => {
  ...
  const sql = page.getByRole('dialog', { name: '数据与系统状态', exact: true }).getByRole('textbox', { name: 'SQL 查询', exact: true })
  await sql.focus()
