...
2026-09-06 回读修订：等待原截止时间内的有效匹配，不因第一条暂未到位的回读立即失败。
调度控制取消自动补发；一次下发失败后停止策略，人工处理并显式启用后才恢复。
...
- 期限内取得新鲜、GOOD 且与期望不符的回读仍保持 `dispatched`，只读等待，不重复
  下发。首条有效不匹配的值、观察时间、质量与来源事件记录到现有审计，仅持久化一次。
  后续有效匹配进入 `readback_confirmed`；原期限到达时，有有效不匹配证据则进入
  `mismatch`，否则进入 `timeout`。`mismatch` 表示期限内曾读到不匹配且未确认到位，
  不声称截止瞬间的设备值。过期、非 GOOD、下发前观测或截止后才取得的结果不能确认成功。
- `(actor, idempotency_key)` 唯一且绑定规范化请求摘要。相同摘要返回原命令；不同摘要
  拒绝为 `IDEMPOTENCY_KEY_REUSED`。
- 冷却以目标实体实例为作用域并由数据库行锁保留；服务重启、并发请求或新运行时实例
  不能绕过它。重启后 `dispatched` 命令继续只读回读，保留原期限及已持久化的不匹配证据；
  到期按上述规则结束，不重新下发。
- 调度意图只进行一次下发。控制拒绝、写入失败、回读超时或最终不匹配均停止该策略，
  取消后续待发意图，保留失败记录；不得因冷却结束、固定节拍或重启而自动补发。
  人工解除失败本身不启用策略，恢复控制必须显式启用。
...
Ticket 11 已把遗留 `/entities/{id}/write` 迁为有限兼容入口：旧全局实体 UUID 只有在其
活动绑定唯一对应到已确认、活动、Neuron 来源的实体实例时才能提交控制命令；不唯一或无法
映射时保存拒绝命令。该入口必须带幂等键，返回命令资源和迁移链接，不再直接调用设备
Adapter，也不再返回同步“写成功”语义。新调用方必须使用实体实例控制端点；兼容入口将在
v1.0 移除。票据 10 起，规则/策略动作仅能提交
`entity_instance_id`、期望值和稳定动作标识。控制规则还必须声明至少一个实体实例输入；规则/策略主体、版本、输入观测与求值输出作为
命令 `origin_evidence` 持久化。它们不包含 Neuron 地址、MQTT topic、QoS 或进程内冷却。
相同触发证据重放命中同一命令；新的触发仍受目标实体实例的持久冷却约束。
...
- 人工、已迁移的 Neuron/RPC 兼容 API、规则、策略和验收共享唯一且可审计的控制语义；
  新消费者不能再直接写物理地址。
