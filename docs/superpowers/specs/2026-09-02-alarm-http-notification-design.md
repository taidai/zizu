...
2026-09-04 修订：通知以状态转换 ID 去重。同一未恢复事件的重复观测不重复通知；恢复后再次发生会创建
新事件并重新发送发生通知，不再受上一事件的 `notification_throttle_seconds` 抑制。该字段暂保留用于
旧配置兼容，不参与跨事件通知判断。同一告警事件只有已生成发生通知任务时才生成配对恢复通知，避免
出现只有恢复、没有发生的孤立消息。
