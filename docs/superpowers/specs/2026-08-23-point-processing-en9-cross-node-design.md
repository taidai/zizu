...
冻结配置写入并记录当前 v0.4.81 容器摘要；完整备份生产数据库，恢复到隔离数据库，验证关键表和记录数量，证明旧容器可以读取原始备份；再对隔离数据库应用 040、启动 rc.1、读取一号机 Neuron 目录并生成计划。任何迁移、恢复、目录或 blocker 失败都保持生产 v0.4.81 不变。
...
因为采用硬切换，回退不尝试让 v0.4.81 读取 Schema 040。恢复需要维护窗口，备份后的生产写入可能丢失；该代价已经由维护者明确接受。
